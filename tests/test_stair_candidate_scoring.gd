...
func _init() -> void:
	...
	# Puerta en (4, 8)
	var dp_a := DoorPair.new("dp_0", 0, 1, Vector2i(4, 8), Vector2i(3, 8), 0)

	var floor_a := DungeonFloorData.new(0, 1337)
	floor_a.grid = grid_a
	floor_a.rooms = [room_a]
	floor_a.door_pairs = [dp_a]
	...
	var floor_b := DungeonFloorData.new(1, 1338)
	floor_b.grid = grid_b
	floor_b.rooms = [room_b]
	...
	assert(vconn.from_cell != dp_a.position_a, "Stair cell must not block door position")
	assert((vconn.from_cell - dp_a.position_a).length() > 1.0, "Stair cell must have margin from door")

	# 5. Validar que los CellGrid tienen los tipos de celda de escalera correctos
	assert(grid_a.get_cell(vconn.from_cell) == CellGrid.CellType.STAIRS_DOWN, "Floor A stair must be STAIRS_DOWN")
	assert(grid_b.get_cell(vconn.to_cell) == CellGrid.CellType.STAIRS_UP, "Floor B stair must be STAIRS_UP")
