...
func _run_test() -> void:
	print("==================================================================")
	print("--- Running test_hanging_fixture_placement ---")
	print("==================================================================")
	...
	for r_ctx in room_contexts:
		...
		for d in directives:
			if d.placement_mode == FixturePlacementModeScript.Mode.HANGING:
				hanging_found = true
				assert(r_geom.floor_cells.has(d.cell), "FAIL: Hanging anchor spot must be above valid room floor cell")
				assert(d.world_position.y >= 2.0, "FAIL: Hanging fixture must be elevated (y >= 2.0)")
				assert(d.placement.normal.is_equal_approx(Vector3.DOWN), "FAIL: Hanging normal must be DOWN")

	assert(hanging_found, "FAIL: At least one hanging fixture should be placed across the rooms")
	print("  [OK] Hanging fixtures (Hanging Lantern): valid suspension anchor, elevation and normal verified.")
	print("[PASS] test_hanging_fixture_placement completed successfully!")
	quit(0)
