...
func test_report_generation() -> void:
	...
	assert(report.find("Topology <-> Geometry") >= 0, "Report must contain Topology <-> Geometry section")
	assert(report.find("Excentricity") >= 0, "Report must contain Excentricity section")
	assert(report.find("edge spatial length") >= 0, "Report must contain edge spatial length metrics")
	assert(report.find("centroid") >= 0, "Report must contain centroid/excentricity metrics")
	assert(report.find("short rate") >= 0, "Report must contain corridor short rate")
...
func test_summary_generation() -> void:
	...
	assert(summary.has("global_corridor_length_p10"), "Summary must have global_corridor_length_p10")
	assert(summary.has("global_corridor_length_median"), "Summary must have global_corridor_length_median")
	assert(summary.has("global_corridor_length_p90"), "Summary must have global_corridor_length_p90")
	assert(summary.has("global_corridor_length_count"), "Summary must have global_corridor_length_count")
	assert(summary.has("global_edge_stretch_p10"), "Summary must have global_edge_stretch_p10")
	assert(summary.has("global_edge_stretch_median"), "Summary must have global_edge_stretch_median")
	assert(summary.has("global_edge_stretch_p90"), "Summary must have global_edge_stretch_p90")
	assert(summary.has("global_edge_stretch_count"), "Summary must have global_edge_stretch_count")
...
func test_snapshot_to_dict() -> void:
	...
	assert(dict.has("nearest_neighbor_cv"), "Dict must have nearest_neighbor_cv")
	assert(dict.has("corridor_short_rate"), "Dict must have corridor_short_rate")
	assert(dict.has("edge_spatial_length_mean"), "Dict must have edge_spatial_length_mean")
	assert(dict.has("edge_spatial_length_min"), "Dict must have edge_spatial_length_min")
	assert(dict.has("edge_spatial_length_max"), "Dict must have edge_spatial_length_max")
	assert(dict.has("edge_spatial_length_stddev"), "Dict must have edge_spatial_length_stddev")
	assert(dict.has("goal_to_centroid_distance"), "Dict must have goal_to_centroid_distance")
	assert(dict.has("boss_to_centroid_distance"), "Dict must have boss_to_centroid_distance")
...
func test_baseline_save() -> void:
	...
	assert(summary.has("worst_nearest_neighbor_cv"), "Summary must have worst_nearest_neighbor_cv")
	assert(summary.has("worst_room_fill_ratio"), "Summary must have worst_room_fill_ratio")
	assert(summary.has("worst_corridor_length_min"), "Summary must have worst_corridor_length_min")
	assert(summary.has("worst_corridor_length_max"), "Summary must have worst_corridor_length_max")
	assert(summary.has("worst_corridor_short_percentage"), "Summary must have worst_corridor_short_percentage")
	assert(summary["worst_nearest_neighbor_cv"].has("seed"), "worst_nearest_neighbor_cv must have seed")
	assert(summary["worst_nearest_neighbor_cv"].has("value"), "worst_nearest_neighbor_cv must have value")
	assert(summary["worst_room_fill_ratio"].has("seed"), "worst_room_fill_ratio must have seed")
	assert(summary["worst_room_fill_ratio"].has("value"), "worst_room_fill_ratio must have value")
	assert(summary["worst_corridor_length_min"].has("seed"), "worst_corridor_length_min must have seed")
	assert(summary["worst_corridor_length_min"].has("value"), "worst_corridor_length_min must have value")
	assert(summary["worst_corridor_length_max"].has("seed"), "worst_corridor_length_max must have seed")
	assert(summary["worst_corridor_length_max"].has("value"), "worst_corridor_length_max must have value")
	assert(summary["worst_corridor_short_percentage"].has("value"), "worst_corridor_short_percentage must have value")
	assert(summary.has("worst_edge_spatial_length_min"), "Summary must have worst_edge_spatial_length_min")
	assert(summary.has("worst_edge_spatial_length_max"), "Summary must have worst_edge_spatial_length_max")
	assert(summary.has("worst_edge_spatial_length_stddev"), "Summary must have worst_edge_spatial_length_stddev")
	assert(summary.has("worst_corridor_short_rate"), "Summary must have worst_corridor_short_rate")
	assert(summary.has("highest_goal_to_centroid_distance"), "Summary must have highest_goal_to_centroid_distance")
	assert(summary.has("highest_boss_to_centroid_distance"), "Summary must have highest_boss_to_centroid_distance")
	assert(summary["worst_edge_spatial_length_min"].has("seed"), "worst_edge_spatial_length_min must have seed")
	assert(summary["worst_edge_spatial_length_min"].has("value"), "worst_edge_spatial_length_min must have value")
	assert(summary["worst_edge_spatial_length_max"].has("seed"), "worst_edge_spatial_length_max must have seed")
	assert(summary["worst_edge_spatial_length_max"].has("value"), "worst_edge_spatial_length_max must have value")
	assert(summary["worst_edge_spatial_length_stddev"].has("seed"), "worst_edge_spatial_length_stddev must have seed")
	assert(summary["worst_edge_spatial_length_stddev"].has("value"), "worst_edge_spatial_length_stddev must have value")
	assert(summary["worst_corridor_short_rate"].has("seed"), "worst_corridor_short_rate must have seed")
	assert(summary["worst_corridor_short_rate"].has("value"), "worst_corridor_short_rate must have value")
	assert(summary["highest_goal_to_centroid_distance"].has("seed"), "highest_goal_to_centroid_distance must have seed")
	assert(summary["highest_goal_to_centroid_distance"].has("value"), "highest_goal_to_centroid_distance must have value")
	assert(summary["highest_boss_to_centroid_distance"].has("seed"), "highest_boss_to_centroid_distance must have seed")
	assert(summary["highest_boss_to_centroid_distance"].has("value"), "highest_boss_to_centroid_distance must have value")
	assert(summary.has("global_corridor_length_p10"), "Summary must have global_corridor_length_p10")
	assert(summary.has("global_corridor_length_median"), "Summary must have global_corridor_length_median")
	assert(summary.has("global_corridor_length_p90"), "Summary must have global_corridor_length_p90")
	assert(summary.has("global_corridor_length_count"), "Summary must have global_corridor_length_count")
