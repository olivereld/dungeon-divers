...
## Genera formas de colisión física (BoxShape3D, Compound Strips, ConcaveTrimesh) para clusters de suelo (Fase M5).
...
func build_collision_for_cluster(
	...
	if cluster == null or (cells.is_empty() and cluster.cells.is_empty()):
		return
	...
	var target_cells: Array = cells if not cells.is_empty() else cluster.cells
	...
	match config.collision_mode:
		_FloorTileConfigScript.CollisionMode.BOX:
		...
		_FloorTileConfigScript.CollisionMode.CONCAVE_TRIMESH:
			# Colisión cóncava exacta a partir de la geometría de la malla
			if cluster.mesh != null:
				var trimesh_shape = cluster.mesh.create_trimesh_shape()
				if trimesh_shape != null:
					cluster.add_collision_shape(trimesh_shape, Transform3D.IDENTITY)

		_FloorTileConfigScript.CollisionMode.COMPOUND_BOX, _:
			# Tiras horizontales contiguas (RLE en X) para reducir drásticamente el número de colliders
			var strips := _extract_horizontal_strips(target_cells)
			for strip in strips:
