...
static func carve_room_shape(
	...
	match shape_family:
		...
		&"chapel":
			_apply_chapel_shape(grid, rect, w, h, entrances, zone_map)

		&"central_nave", &"nave":
			_apply_central_nave_shape(grid, rect, w, h, entrances, zone_map)

		&"niched_hall", &"niches":
			_apply_niched_hall_shape(grid, rect, w, h, entrances, zone_map)
	...
	# Assign circulation and perimeter zones for remaining walkable floor cells
...
static func _apply_chapel_shape(grid: CellGrid, rect: Rect2i, w: int, h: int, entrances: Array[Vector2i], zone_map: _ZoneMapScript) -> void:
	# Chapel: Recessed corners on southern/side entrance, apse focal zone on northern wall
	var cut: int = 1 if (w <= 7 or h <= 7) else 2
	for cx in range(cut):
	...
	# Apse focal zone in the upper-center
	var apse_center := rect.position + Vector2i(w / 2, 1)
	zone_map.set_zone(apse_center, &"focal")
	if w >= 8:
		zone_map.set_zone(apse_center + Vector2i(-1, 0), &"focal")
		zone_map.set_zone(apse_center + Vector2i(1, 0), &"focal")

static func _apply_central_nave_shape(grid: CellGrid, rect: Rect2i, w: int, h: int, entrances: Array[Vector2i], zone_map: _ZoneMapScript) -> void:
...
static func _apply_niched_hall_shape(grid: CellGrid, rect: Rect2i, w: int, h: int, entrances: Array[Vector2i], zone_map: _ZoneMapScript) -> void:
