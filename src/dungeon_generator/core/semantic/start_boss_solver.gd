...
func _resolve_boss_room(rooms: Array = [], start_id: int = -1, depth_map: Dictionary = {}) -> int:
	# 1. room_type == &"boss" o &"goal"
	for r in rooms:
		if (r.room_type == &"boss" or r.room_type == &"goal") and r.id != start_id:
			return r.id

	# 2. Sala de mayor depth desde start_id
	var max_depth: int = -1
	...
	for r in rooms:
		...
		if d > max_depth:
			...
			# Desempate determinista: mayor room_id
			if r.id > best_boss_id:
				best_boss_id = r.id

	if best_boss_id != -1:
		return best_boss_id

	# Fallback si no hay depth_map o solo hay 1 sala
	for r in rooms:
		if r.id != start_id:
			return r.id
